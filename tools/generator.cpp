...
int main(int argc, char** argv) {
  GenOptions opt;
  if (!parse_cli_gen(argc, argv, opt)) {
    return 2;
  }
  if (opt.cpu >= 0) {
  ...
  const uint32_t flows = std::max(opt.flows, 1u);
  ...
  while (now_ns() < end_target) {
    for (uint32_t i = 0; i < opt.batch; ++i) {
      const uint32_t flow = (flows == 1) ? 0u : uint32_t(sent + i) % flows;
    ...
    const int rc = sendmmsg(fd, msgs.data(), opt.batch, 0);
    if (rc < 0) {
      if (errno == EINTR) {
        continue;
      }
  ...
  std::cout << "sent=" << sent << " secs=" << secs
            << " pps=" << (secs > 0.0 ? (double(sent) / secs) : 0.0) << "\n";
